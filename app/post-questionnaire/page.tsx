    ...
    <>
      ...
      <div style={{ fontSize: 14, color: "#555", lineHeight: 1.9, marginBottom: 32 }}>
        ...
        <p style={{ margin: "0 0 10px" }}>
          ...
          您認為最能描述<strong style={{ color: "#3a3a3a" }}>剛剛表現出來的自己</strong>的形容詞對，並在每對之間選擇最符合您的位置。
        </p>
      ...
      <div style={{ display: "flex", gap: 12 }}>
        ...
        <button
          ...
          style={{
            ...
            backgroundColor: ready && !submitting ? "#CCD5AE" : "#e5e5e5",
            color: ready && !submitting ? "#3a3a3a" : "#aaa",
          }}
        >
          {submitting ? "提交中⋯⋯" : "確認送出"}
        </button>
...
function PostQuestionnaireContent() {
  ...
  const handleSubmit = async () => {
    if (getSelectedCount(answers) !== REQUIRED || submitting) return;
    setSubmitting(true);
    setSubmitError(false);

    try {
      const res = await fetch("/api/post-questionnaire", {
        method: "POST",
